...
function PeopleContent(props) {
  ...
  return (
    props.peopleList.map((people) =>
      <div key={people.idEmpresaProveedora}>
        <div className="media text-muted pt-3 border-top border-gray">
          ...
          <div className="media-body pb-3 mb-0 small lh-125">
            <strong className="d-block text-gray-dark">{people.nombreEmpresa}</strong>'
            <ul className="list-group float-left mt-2 w-75">
              <li className="list-group-item">
                <strong>Direccion</strong> : {people.direccionEmpresa}
              </li>
              <li className="list-group-item">
                <strong>NIT</strong> : {people.nitempresa}
              </li>
              <li className="list-group-item">
                <strong>Monto Permitido</strong> : ${people.montoPermitido}
              </li>
            </ul>
            <div className="float-right">
              <a href={ '/add_company?action=edit&id=' + people.idEmpresaProveedora} className="btn btn-link btn-sm">
                Editar
              </a>
    ...
    <Layout isAuth={props.isAuth}>
      <div className="nav-scroller bg-white shadow-sm">
        <nav className="nav nav-underline">
          ...
          <a className="p-2 nav-link active disabled" href="#">Nueva empresa proveedora</a>
        </nav>
      </div>
      <div className="d-flex align-items-center p-3 mb-3 text-white-50 bg-purple rounded shadow-sm">
        ...
        <div className="lh-100">
          <h6 className="mb-0 text-white lh-100">Administrar Empresas Proveedoras</h6>
        </div>
      </div>
      <div className="my-3 p-3 bg-white rounded shadow-sm">
        <h6 className="pb-2 mb-0 float-left">Todos Empresas</h6>
        <a href='/add_company' className="p-2 mb-2 btn btn-primary float-right">
          <FontAwesomeIcon icon={faPlus} />
          <span className="ml-2 ">Nuevo Empresa</span>
        </a>
...
People.getInitialProps = async ctx => {
  ...
  const url = `${process.env.API_URL}/EmpresaProveedoras`
  try {
