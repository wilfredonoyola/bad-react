      ...
      <div key={people.idRol}>
        <div className="media text-muted pt-3 border-top border-gray">
          ...
          <p className="media-body pb-3 mb-0 small lh-125">
            <strong className="d-block text-gray-dark">{people.nombreRol}</strong>
          </p>
        </div>
      </div>
    )
    ...
    <Layout isAuth={props.isAuth}>
      <div className="nav-scroller bg-white shadow-sm">
        <nav className="nav nav-underline">
          ...
          <a className="p-2 nav-link active disabled" href="/people">Roles</a>
        </nav>
      </div>
      <div className="d-flex align-items-center p-3 mb-3 text-white-50 bg-purple rounded shadow-sm">
        ...
        <div className="lh-100">
          <h6 className="mb-0 text-white lh-100">Administar Roles</h6>
        </div>
      </div>
      <div className="my-3 p-3 bg-white rounded shadow-sm">
        <h6 className="pb-2 mb-0 float-left">Todos Roles</h6>
        <a href='/ayuda' className="p-2 mb-2 btn btn-primary float-right">
...
People.getInitialProps = async ctx => {
  ...
  const url = `${process.env.API_URL}/Rols`
